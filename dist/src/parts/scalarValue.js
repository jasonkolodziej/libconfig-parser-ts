...
const StringParser = Parse.query(function* () {
    ...
    let rest = [];
    while (!done) {
        rest.push(yield Parse.char(char => !/\"|\\/.test(char), "all chars except for \" or \\")
            .many().text());
...
const AdjacentString = Parse.query(function* () {
    ...
    while (true) {
        ...
        if (!a.wasSuccessful && a.wasSuccessful === b.wasSuccessful) {
            break;
        }
...
//# sourceMappingURL=scalarValue.js.map